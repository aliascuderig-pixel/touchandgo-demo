...
9. **Conclusione soggiorno** (`ConcludeScreen`) — quando il turista ha finito di fare acquisti, consolida tutti gli acquisti "in sospeso" per destinazione in un unico ordine di ritiro (`ShippedScreen`). **È qui, e solo qui, che avviene il calcolo finale vero** (peso/volume combinato del gruppo, una sola fee di servizio — vedi "Prezzo consolidato per gruppo di spedizione") **e il pagamento reale** (Stripe Checkout, vedi "Pagamento reale con Stripe Checkout") — gli acquisti passano a "ritirato" solo dopo che il pagamento è stato verificato lato server (percorso legacy, oggi affiancato dal flusso più granulare a 4 stati descritto sotto).
...
#### Persistenza della stima (`dutyEstimateShown`, settembre 2026)

**Cambiato da questa modifica**: fino a qui `state.dutyEstimate` era **solo stato client** — mai salvato sul record dell'acquisto in `save-purchase.js`, quindi il CRM/lo staff non avevano modo di vedere quale stima fosse stata mostrata a un turista per un dato acquisto. Primo passo per poter eventualmente confrontare in futuro la stima con un dazio reale riportato (nessun confronto costruito ora — solo la registrazione del dato).

- **Nuovo campo opzionale `dutyEstimateShown`** sul record dell'acquisto: il testo **esatto** della stima mostrata al turista al momento della conferma (`state.dutyEstimate`), o `null` se la stima non è mai arrivata, è fallita, o non è stata richiesta — coerente col vincolo 1 "mai bloccante" già esistente (vedi sopra): un acquisto non deve mai poter fallire o essere ritardato per colpa della stima dazi, né prima né ora.
- **Due punti di scrittura, entrambi in `dist/assets/app.js`**: il percorso turista self-service (`ChooseAddressScreen`, costruzione dell'item prima di `syncPurchaseToCRM()`) legge `state.dutyEstimate` così com'è in quel momento; il percorso "Genera spedizione" dell'area partner (`submitPartnerGeneratedShipment()`) non chiama mai `refreshDutyEstimate()` — scrive quindi sempre `dutyEstimateShown: null`, stesso significato "non richiesta" del percorso turista, mai un errore.
- **`netlify/functions/save-purchase.js`: nessuna modifica obbligatoria per accettarlo** — stessa situazione già documentata per `country`/`city` (vedi "Paese e città reali della spedizione" più sotto) e per `objectName`/`category`/`material`/`hsCode`: `isValidPurchase()` non li ha mai validati, accetta già qualunque campo extra nel payload, e l'handler scrive l'intero oggetto `item` così com'è arrivato (`purchases.setJSON(item.id, item)`). Aggiunto solo un commento di documentazione nel file, nessuna riga di logica — coerente con "solo aggiungere il salvataggio di quel valore, nessuna modifica alla logica di calcolo/visualizzazione esistente".
- **Isolamento dal prezzo, verificato esplicitamente anche qui**: stesso principio già garantito per `state.dutyEstimate` lato client (vedi vincolo 2 sopra) — `dutyEstimateShown` non entra mai nel calcolo di `item.price` in `save-purchase.js`, verificato sia confrontando il prezzo salvato con e senza stima disponibile (deve essere identico), sia con lo stesso tipo di grep statico già usato per l'isolamento lato client (nessuna riga che menzioni `dutyEstimateShown` vicino a `COMMISSION_RATE`/`creditIssuedAmount`/logica di prezzo).

**Test**: 11 nuovi.
- `netlify/functions/__tests__/save-purchase.duty-estimate.test.js` (7): una stima disponibile viene salvata byte-per-byte identica (anche con testo lungo/multilingua/emoji); `dutyEstimateShown: null` e il campo del tutto assente dal payload sono entrambi accettati senza errore; una risincronizzazione dello stesso acquisto (cambio status) non altera il valore già scritto; il prezzo salvato è identico con o senza stima; grep statico sul corpo del file che conferma nessun collegamento economico.
- `dist/assets/__tests__/duty-estimate-persistence.test.js` (4, end-to-end client con `app.js` reale in jsdom, stessa tecnica di `real-country-city.test.js`: flusso di acquisto completo fino al payload effettivo inviato a `/.netlify/functions/save-purchase`): una stima disponibile al momento della conferma finisce esattamente su `item.dutyEstimateShown`; un errore HTTP o di rete nella chiamata a `estimate-duty` non impedisce l'acquisto e salva `null`; il prezzo dell'acquisto è identico con o senza stima disponibile.
- Confermato che i 6 test di isolamento preesistenti in `dist/assets/__tests__/duty-estimate.test.js` (vincolo 2, stima mai vicina a `priceFor()`/`priceQuotes()`/`bracketPrice()`/`shippingCost()`/`grandTotal` in `app.js`) continuano a passare invariati dopo questa modifica.
...
- **Due punti di applicazione**, entrambi con lo stesso controllo prima di qualunque mutazione di stato:
  ...
  - **`ConcludeScreen()`** — il bottone di conferma/pagamento (Stripe Checkout, vedi sezione "Pagamento reale con Stripe Checkout" più sotto) che consolida tutti gli acquisti "in sospeso": stesso controllo, `state.identifyReturnTo = "conclude"`.
...
- **Test**: `netlify/functions/__tests__/save-shipment-group.test.js` — gruppo valido salvato con chiave = bookingCode, validazione dei dati (itemIds vuoto, peso/prezzo fuori range), metodo non-POST rifiutato, due gruppi diversi non si sovrascrivono. La regola anti-frode 8 (numero insolito di oggetti) ha il proprio file dedicato, vedi sotto.
...
Prima di questa modifica, un turista offline (o la cui chiamata di classificazione falliva per motivi di rete a metà percorso) restava **bloccato**: `DestinationScreen` mostrava un errore permanente ("Sei offline...") e non c'era alcun modo di proseguire finché la connessione non tornava — un problema reale per un cliente fermo in un negozio senza segnale. Ora il turista può completare l'intero percorso fino al QR anche offline, con un prezzo **provvisorio** basato sulla categoria scelta a mano, confermato (o corretto) al ritorno della connessione.

#### 1. Dieci categorie, con sottocategorie solo lato client

`CLASSIFY_SCHEMA` (`dist/assets/app.js`) elenca ora **10** categorie (prima 9): aggiunta **"Attrezzatura sportiva"**. Le sottocategorie (`CATEGORY_SUBCATEGORIES`, stesso file) esistono **solo lato client**, per il selettore offline sotto — **non** fanno parte dello schema inviato all'AI: obbligare il modello a scegliere anche una sottocategoria aggiungerebbe un vincolo che non aiuta la classificazione reale (l'AI vede la foto/descrizione vera, non ha bisogno di un elenco chiuso) e complicherebbe il prompt senza un beneficio proporzionato. "Altro" non ha sottocategorie: è già la categoria residuale.

`netlify/lib/category-stats.js` (nuovo, condiviso da `save-purchase.js` e `category-averages.js`, vedi sotto) duplica lo stesso elenco di 10 categorie lato server — nessun modulo condiviso tra client e funzioni serverless in questo repository, stesso principio già in uso ovunque (es. `netlify/lib/guest-mode.js` copiato identico in `touchandgo-internal`): se `CLASSIFY_SCHEMA` cambia, questo elenco va aggiornato di conseguenza.

#### 2. Cache locale dei valori medi per categoria

**`netlify/functions/category-averages.js`** (nuovo, `GET`, rate-limited) restituisce, per ciascuna delle 10 categorie, il peso e le dimensioni medi calcolati sui `weightKg`/`dims` degli acquisti reali nello store `purchases`:

- **Stessa soglia di campione minimo delle regole anti-frode statistiche** (`isCategoryOutlier()`, vedi "Blocco manuale e segnalazioni anti-frode" sotto): **5** acquisti. Sotto quella soglia una media non è un riferimento significativo — la categoria usa invece un **default dichiarato** (`CATEGORY_DEFAULTS` in `netlify/lib/category-stats.js`), una stima a spanne di un oggetto "tipico" per categoria (es. Ceramica: 1,5kg, 25×20×15cm; Attrezzatura sportiva: 2,5kg, 60×30×20cm — vedi il commento nel file per l'elenco completo e il ragionamento). Peso e ciascuna delle tre dimensioni sono decisi **indipendentemente**: un campione sufficiente per il peso ma non per un'altezza (dati storici parziali) usa comunque il default solo per quel singolo campo.
- **Estratta, non duplicata**: `isCategoryOutlier()` (regole anti-frode 3/6 in `save-purchase.js`) e la nuova `categoryAverage()` condividono ora la stessa implementazione in `netlify/lib/category-stats.js` — prima di questa modifica la logica statistica viveva solo in `save-purchase.js`; estrarla evita due copie che potrebbero divergere nel tempo. Comportamento di `isCategoryOutlier()` invariato (stesso 3x/campione 5), solo il codice si è spostato.
- **`dist/assets/app.js`, `refreshCategoryAverages()`** — scarica la risposta e la salva in `localStorage` (`tg_category_averages_cache`, con `cachedAt`). Chiamata **mentre l'app è online**: all'avvio, al ritorno dell'evento `online`, e ogni 3 minuti — **esattamente gli stessi punti di innesco già usati da `processPendingSyncQueue()`** (stesso `setInterval`, stessa logica "fire and forget": un fallimento lascia semplicemente la cache com'era, mai un errore mostrato al turista). **Questa è l'UNICA fonte usata per il prezzo provvisorio offline — mai una chiamata di rete nel momento in cui serve davvero**, che per definizione può essere il momento sbagliato per riuscirci.
- **Fallback anche senza cache mai scaricata** (`CATEGORY_AVERAGES_FALLBACK` in `dist/assets/app.js`) — stessi identici valori di `CATEGORY_DEFAULTS` lato server, duplicati deliberatamente: se questo dispositivo non è mai stato online da quando l'app è stata aperta la prima volta (o lo storage è stato cancellato), il percorso offline funziona comunque, con lo stesso default ragionevole invece di un errore.

#### 3. Selezione categoria e prezzo provvisorio

- **Due punti di ingresso**, entrambi verso `state.screen = "offline-classify"`: (a) `DestinationScreen`, se `state.isOffline` è già vero quando si tappa "Analizza"; (b) `runClassification()`, se la chiamata a `classify()` fallisce **per un motivo di rete vero e proprio** a metà percorso. Il secondo caso richiede di distinguere un fallimento di rete da un errore reale (es. **401** chiave API, 429 rate limit, 500) — `classify()` ora marca esplicitamente `err.isNetworkError = true` quando la `fetch` stessa non arriva a un server (offline, DNS, connessione caduta), invece di affidarsi al testo del messaggio d'errore (che varia per browser: "Failed to fetch", "NetworkError when attempting...", "Load failed"...). Solo `err.isNetworkError` (o `state.isOffline` già vero) porta al percorso offline; qualunque altro errore resta un vero errore mostrato come prima.
- **`OfflineClassifyScreen()`** — selettore in due passi nella stessa schermata: categoria, poi (se ne ha) sottocategoria. "Altro" (senza sottocategorie) conferma subito alla scelta della categoria.
- **`buildProvisionalResult(category, subcategory)`** — costruisce un oggetto nello **stesso identico formato** dell'output AI reale (stessi campi di `CLASSIFY_SCHEMA`), popolato con `weight_kg`/`length_cm`/`width_cm`/`height_cm` dalla cache dei valori medi (punto 2), `value_eur: 0` (nessuna stima di valore possibile senza classificazione reale — mai inventata), `confidence: "bassa"`. Costruire lo stesso formato permette di **riusare quasi invariati** `ResultScreen()`/`priceFor()`/il costruttore dell'item in `ChooseAddressScreen()` — il percorso offline non duplica quella logica, la attraversa con un risultato diverso.
- **`state.resultIsProvisional`** distingue un risultato costruito così da uno vero. `ResultScreen()` mostra, in cima, un banner **sempre visibile** (`"📡 Stima provvisoria — sarà confermata al ritorno della connessione"`) quando `resultIsProvisional` è vero — **vincolo non negoziabile**: il prezzo/risultato provvisorio non deve mai essere scambiato per uno reale in nessun punto dell'interfaccia.
- Da lì il turista prosegue **esattamente come nel percorso normale** — destinazione, indirizzo, conferma — fino al QR (`generateBookingCode()`, già puramente client-side): nessuna di queste schermate richiede rete.

#### 4. L'item salvato

Il costruttore dell'item in `ChooseAddressScreen()` (invariato per il percorso online) aggiunge, quando `state.resultIsProvisional` è vero:

- **`pendingRealClassification: true`** — marcatore letto dalla riconciliazione (punto 5).
- **`provisionalCategory`/`provisionalSubcategory`** — la scelta manuale del turista, conservata anche dopo la riconciliazione per riferimento.
- **`destinationZone`** — scritto invece **sempre**, anche per un acquisto online: è il nome `DESTINATIONS`/`priceFor()` (es. `"Italia"`) usato per **questo** prezzo. Necessario perché la riconciliazione (che avviene più tardi, quando il turista ha già lasciato quella schermata) deve ricalcolare lo **stesso** prezzo con lo stesso criterio di zona, senza dipendere dallo stato di navigazione corrente.

`photo`/`textDescription` (già catturati identicamente nel percorso online, per la stima dell'imballo) sono ciò che rende possibile riclassificare più tardi: nessun campo nuovo necessario per questo.

#### 5. Riconciliazione al ritorno della connessione

**`processPendingReclassifications()`** (`dist/assets/app.js`) — meccanismo **parallelo** alla coda di ritentativo sync CRM (`enqueueFailedSync`/`processPendingSyncQueue`), non un'estensione di quella: quella coda ritenta un semplice `POST` con lo stesso payload; qui serve invece rieseguire una **vera classificazione** (foto/descrizione salvate) e poi ricalcolare il prezzo — un'azione diversa, non un retry dello stesso salvataggio. Stessi punti di innesco per coerenza (avvio, evento `online`, stesso intervallo di 3 minuti — vedi punto 2).

- Per ogni item di `state.pendingItems` con `pendingRealClassification: true`: richiama `classifyImage()`/`classifyText()` con la foto/descrizione già salvata (`parseDataUrl()` estrae base64/mediaType dal data URL prodotto da `compressImage()`), poi **`recomputeReconciledPrice()`** ricalcola il prezzo con lo **stesso tier/sconto già applicato** all'acquisto (`item.pricingTier`/`item.partnerDiscountAmount`, decisi al momento della conferma) — **mai** lo stato globale *corrente* del turista, che nel frattempo può essere cambiato (es. abbonamento attivato dopo, promo scaduta): si confronta "stesso accordo economico, dati reali" contro "stesso accordo economico, stima provvisoria".
- Aggiorna `category`/`hsCode`/`material`/`weightKg`/`dims`/`itemValue`/`price`, azzera `pendingRealClassification`, imposta `reclassifiedAt`, e **risincronizza il record col CRM** (`syncPurchaseToCRM()`, invariata).
- **Sincronizzato anche in `purchaseHistory`** — stesso pattern già in uso per `deliveryConfirmedAt`/`finalizeShippedGroups()` (vedi "Pagamento reale con Stripe Checkout"): `pendingItems` e `purchaseHistory` sono due array indipendenti dopo un giro di salvataggio in `localStorage`, occorre aggiornare esplicitamente anche la voce corrispondente in `purchaseHistory`, per `id`, se è un oggetto diverso.
- **Un fallimento (offline a metà, errore AI transitorio) non scrive nulla**: l'item resta `pendingRealClassification: true`, ritentato al prossimo innesco — nessuna scrittura parziale, nessun prezzo a metà aggiornato.
- **Notifica di riconciliazione, mai un cambio di prezzo silenzioso** — se il prezzo reale si discosta di oltre il **20%** (relativo) da quello provvisorio (`PRICE_RECONCILIATION_THRESHOLD`), l'item riceve `priceReconciliationNotice: { oldPrice, newPrice, reconciledAt }`. Soglia scelta perché la stima provvisoria si basa deliberatamente su una **media di categoria** (non l'oggetto specifico): uno scostamento moderato è normale e atteso, non degno di interrompere il turista; il 20% intercetta solo un cambiamento davvero sorprendente (es. la foto era in realtà di un oggetto molto più grande/pesante della media della categoria scelta). `priceReconciliationBanner()` in `PurchaseHistoryList()` (stesso pattern di `pickupPointUpdateBanner()`/`markPickupPointSeen()`) mostra un banner dismissibile con un tap — a differenza del banner del punto di ritiro non serve alcuna conferma lato CRM, il record è già sincronizzato: il banner informa solo il turista.

#### Vincoli rispettati

- **Nessuna dipendenza npm nuova** — nessun modello AI locale/TensorFlow.js, coerente con la disciplina già rispettata ovunque in questo repository.
- **La logica di classificazione online esistente non è stata toccata** — `runClassification()` per il percorso di successo è invariata (solo `resultIsProvisional = false` esplicito, per non lasciare un flag residuo da un tentativo offline precedente); il ramo `catch` distingue ora rete/altro invece di trattare ogni fallimento allo stesso modo, ma un errore non di rete produce esattamente lo stesso comportamento di prima.
- **Il prezzo provvisorio non è mai scambiato per uno reale** — banner sempre visibile in `ResultScreen()` finché `resultIsProvisional` è vero.

#### Verifica

**`netlify/functions/__tests__/category-averages.test.js`** (9 test) — tutte e 10 le categorie sempre presenti; media reale usata con campione ≥5; default dichiarato usato sotto soglia (mai la media di un campione troppo piccolo); categorie indipendenti tra loro; peso e una singola dimensione mancante decisi indipendentemente; `computedAt` valido; rate limit; metodo non-GET rifiutato.

**`dist/assets/__tests__/offline-classify.test.js`** (9 test, end-to-end con `app.js` reale in jsdom) — (1) offline: selezione categoria → sottocategoria → QR, usando la cache locale, **mai** una chiamata a `classify.js` durante l'intero percorso; "Altro" conferma subito, senza passo intermedio; (2) verificato esplicitamente nello stesso test del punto 1 (nessuna chiamata di rete); (3) senza cache mai scaricata (`localStorage` vuoto): usa il fallback dichiarato, nessun errore che blocca il percorso; (4) riconciliazione: un item `pendingRealClassification` viene classificato e il prezzo ricalcolato, sincronizzato anche nella voce indipendente in `purchaseHistory`; nessun item da riconciliare → nessuna chiamata, nessun crash; (5) una differenza di prezzo significativa genera `priceReconciliationNotice`; una differenza trascurabile (stesso peso/dimensioni) no; (6) il percorso online esistente: una classificazione reale riuscita imposta `resultIsProvisional: false`; un errore non di rete (es. 401) resta un vero errore, non passa mai al percorso offline.

**`dist/assets/__tests__/support-trail.test.js`** — il test preesistente sul vecchio comportamento bloccante offline è stato aggiornato al nuovo comportamento (passa a `offline-classify`, nessuna voce "Errore:" nel trail).

Suite completa del repository verde dopo la modifica (`npm test`, 318/318 nella root; `node --test` nella sottocartella `router/`, 18/18, sito Netlify indipendente, non toccato).
...
### Pagamento reale con Stripe Checkout (settembre 2026)

**Cambiato da questa modifica**: fino a qui Touch&Go non aveva alcun pagamento reale integrato — il click su "Conferma e paga" in `ConcludeScreen` confermava sempre, incondizionatamente, con un semplice `setTimeout`. Da questa modifica il pagamento è **reale**, elaborato da Stripe (oggi in **modalità test**: `STRIPE_PUBLISHABLE_KEY`/`STRIPE_SECRET_KEY` impostate su questo sito sono chiavi `pk_test_...`/`sk_test_...` — nessun addebito reale finché non verranno sostituite con chiavi live). Il vincolo già esistente prima di questa modifica resta identico e non negoziabile: il pagamento deve avvenire **prima** di marcare qualunque oggetto come `"ritirato"` e sincronizzarlo, mai dopo — se il pagamento fallisce o viene annullato, nessun oggetto viene marcato.

**Perché Stripe Checkout (pagina ospitata da Stripe) e non un modulo carta custom**: un modulo di pagamento scritto a mano gestirebbe dati di carta reali nel nostro frontend, con tutto l'onere di conformità PCI-DSS che ne consegue (mai accettabile per un prototipo pre-seed). Checkout è invece una pagina interamente ospitata su un dominio Stripe: il browser del turista invia i dati carta **direttamente a Stripe**, mai al nostro server o al nostro codice — a noi arriva solo un `session_id` di conferma. Scelta esplicita di Giuseppe, con un vincolo altrettanto esplicito: **nessuna libreria npm `stripe`** (coerente con "zero dipendenze npm nuove" già rispettato ovunque in questo repository) — tutte le chiamate sono `fetch` dirette alle REST API pubbliche di Stripe (`api.stripe.com`), e nemmeno lo script client-side Stripe.js è caricato: il client si limita a un redirect (`window.location`) verso l'URL della sessione che il server restituisce.

**Il totale non è mai un numero di cui fidarsi dal client**: `netlify/functions/create-checkout-session.js` riceve gli item **grezzi** in sospeso (peso, dimensioni, tier di prezzo, sconto partner, paese di destinazione) — mai un `total`/`amount` già calcolato — e lo ricalcola da zero con `consolidatedGroupPriceForItems()` (`netlify/lib/pricing.js`), **stessa identica formula** di `consolidatedGroupPrice()` in `dist/assets/app.js`, duplicata deliberatamente qui: client e server non condividono moduli in questo repository (stessa tecnica già documentata per `DESTINATIONS_MAP`/`PARTNER_PLAN_TO_PRICING_TIER`). Un eventuale campo `total` che arrivasse comunque nel corpo della richiesta viene semplicemente ignorato — nessun percorso del codice lo legge. Un item con peso fuori range (≥50kg) o `pricingTier` non riconosciuto viene rifiutato con `400` prima ancora di calcolare alcunché.

**Flusso end-to-end**:

1. **`ConcludeScreen`, click su "Conferma e paga €X"** — dopo il gate identità (`hasValidIdentity()`, invariato): il pulsante si disabilita ("Reindirizzo al pagamento…") e chiama `createCheckoutSession(itemsByDest)`, che invia a `create-checkout-session.js` gli item grezzi di ciascun oggetto in sospeso, raggruppati per destinazione (`addressLabel`) — stessa unità di consolidamento di `consolidatedGroupPrice()`: un gruppo = una riga (`line_item`) nella sessione Checkout, con lo stesso identico totale già mostrato al turista nel riepilogo sopra il pulsante.
2. **`create-checkout-session.js`** — rate limiting (stesso schema di tutte le altre function), valida gli item, ricalcola il totale per gruppo (vedi sopra), crea la sessione via `POST https://api.stripe.com/v1/checkout/sessions` (form-urlencoded, `mode=payment`, un `line_items[i]` per gruppo, `success_url=<origine>/?session_id={CHECKOUT_SESSION_ID}`, `cancel_url=<origine>/`) e restituisce `{ url }`. L'origine (schema+host) è letta dall'header `Origin`/`Referer` della richiesta, **mai hardcodata**: funziona automaticamente su produzione, spazio ospite o una deploy preview, senza bisogno di un dominio scritto nel codice (stesso principio già applicato scegliendo `partnership@touchandgo.it` invece di inventare un indirizzo, e la ragione per cui i domini rinominati `benevolent-longma-57c78a`/`cute-moxie-cd1e4b` sono stati aggiornati in tutto il repository in una modifica precedente).
3. **Il client fa `window.location.href = url`** (isolato in `redirectToCheckout()`, solo per restare testabile) — il turista lascia l'app ed entra sulla vera pagina di pagamento Stripe. In modalità test: carta `4242 4242 4242 4242`, qualunque data futura/CVC per un pagamento che va sempre a buon fine.
4. **Ritorno da Stripe** — `success_url` riporta il turista sull'app con `?session_id=...`: `captureCheckoutSessionFromUrl()` (stesso pattern di `capturePartnerCode()`/`captureModeFromUrl()`/`capturePromoCode()`/`captureReviewFromUrl()`, un parametro letto una volta all'avvio) imposta `state.checkoutVerifying = true` **prima** del primo render, così `CheckoutVerifyingScreen()` compare subito (corto circuito in `render()`, come già fa `state.screen === "onboarding"`) invece di mostrare per un istante la schermata con lo stato "in sospeso" non ancora aggiornato.
5. **`verifyCheckoutSession()`** chiama `verify-checkout-session.js`, che interroga **davvero** Stripe (`GET /v1/checkout/sessions/{id}`) e restituisce `paid: true` **solo se** `payment_status === "paid"` — il solo ritorno del browser sulla `success_url` non è mai sufficiente (un `session_id` può essere manomesso, riusato o vecchio). La function è **fail-closed ovunque**: `session_id` mancante, chiave Stripe non configurata, sessione inesistente, eccezione di rete → sempre e solo `paid: false`, mai un errore che il client non saprebbe come interpretare.
6. **Solo `paid: true` fa procedere** con `finalizeShippedGroups()` — **esattamente** la stessa logica di marcatura che prima di questa modifica girava incondizionatamente nel `setTimeout`: marca ogni oggetto `"ritirato"`, assegna il codice gruppo, salva i gruppi (`saveShipmentGroupToCRM`) e sincronizza ogni oggetto (`syncPurchaseToCRM`), svuota `state.pendingItems`, passa a `ShippedScreen`. Qualunque altro esito (`paid: false`, errore di rete, nessuna risposta) mostra un messaggio (`state.checkoutError`, banner `.alert` in `ConcludeScreen`) e **non tocca** `state.pendingItems`/`state.purchaseHistory`.
7. **`cancel_url` (pagamento annullato dal turista su Stripe)** — nessun `session_id` nell'URL di ritorno: `captureCheckoutSessionFromUrl()` non trova nulla da leggere, `verifyCheckoutSession()` non viene nemmeno chiamata. Stesso comportamento di un pagamento mai avvenuto, per costruzione (non serve un caso speciale).
8. In ogni caso (pagato o no) l'URL viene ripulito da `?session_id=` con `history.replaceState()` — non deve mai restare riusabile/condivisibile nella barra indirizzi.

**Bug scoperto e corretto in questo stesso pezzo**: `finalizeShippedGroups()` gira ora dopo una **navigazione completa** (andata e ritorno da Stripe), quindi in un nuovo caricamento della pagina — `loadPending()`/`loadHistory()` ricaricano `state.pendingItems`/`state.purchaseHistory` come **due array indipendenti** da due chiavi `localStorage` separate (a differenza del percorso "aggiungi acquisto", dove lo stesso oggetto viene messo in entrambi gli array con un doppio `push()`, restando quindi sempre lo stesso riferimento per tutta la sessione). Senza una correzione esplicita, marcare `"ritirato"` sull'oggetto in `pendingItems` non si sarebbe mai riflesso sulla voce corrispondente in `purchaseHistory` (stesso `id`, oggetto diverso) — il turista avrebbe visto per sempre l'oggetto come "in sospeso" nel proprio storico, nonostante il pagamento e il ritiro fossero già confermati. `finalizeShippedGroups()` ora cerca esplicitamente la voce corrispondente per `id` in `purchaseHistory` e la aggiorna anche lì, se è un oggetto diverso.

**`ShippedScreen`/footer aggiornati per riflettere la realtà**: titolo invariato ("Ordine confermato e pagato"), ma la riga del totale ora dice *"Pagamento di €X confermato via Stripe (modalità test)"* e la nota prototipo *"il pagamento è reale (in modalità test Stripe), ma nessuna richiesta è stata ancora inviata a un vero corriere"* — non più "simulato". Stesso aggiornamento nel tagline del footer (IT/EN): *"Pagamento reale via Stripe (modalità test) · Ritiro corriere ancora simulato"*.

**Verifica**: 44 nuovi test.
- `netlify/functions/__tests__/create-checkout-session.test.js` (18): il totale per un singolo oggetto e per un gruppo di più oggetti (una sola fee) corrisponde esattamente al calcolo server-side; due destinazioni generano due `line_items` distinti la cui somma è il totale complessivo; un `total`/`amount` mandato dal client viene sempre ignorato (stesso identico importo calcolato, mai quello proposto); lo sconto partner è sottratto dalla fee; validazione item (peso fuori range, `pricingTier` sconosciuto, destinazione mancante) → `400`, nessuna chiamata Stripe; origine da header `Origin`/`Referer`, mai hardcodata, `400` se assente; `mode=payment`/valuta `eur`/quantità 1 fissi; la secret key viaggia come Bearer token, mai nel body/URL; rate limit; chiave mancante → `500`; errore Stripe propagato con lo stesso status.
- `netlify/functions/__tests__/verify-checkout-session.test.js` (19): `payment_status: "paid"` → `paid:true`; ogni altro valore (incluse varianti di maiuscole/minuscole, stringa vuota, `null`/`undefined`) → sempre `paid:false`, mai `true`; session_id URL-encoded nel path; fail-closed su sessione inesistente (404), `session_id` mancante/vuoto, chiave non configurata, eccezione di rete, risposta Stripe malformata; rate limit.
- `dist/assets/__tests__/checkout-payment.test.js` (7, end-to-end lato client, app.js reale in jsdom): il payload inviato a `create-checkout-session` contiene solo gli item grezzi (mai un `total`/`price precalcolato`) e il redirect avviene solo dopo la risposta del server; un fallimento nella creazione della sessione non marca nulla e mostra un errore; un ritorno con `payment_status: "paid"` marca **tutti** gli oggetti (inclusa la sincronizzazione dello storico, vedi bug corretto sopra) e svuota la coda; un ritorno con `paid:false` o un errore di verifica non marca **nulla**; durante la verifica compare `CheckoutVerifyingScreen`, mai la schermata normale con lo stato vecchio; un ritorno senza `session_id` (cancel) non chiama nemmeno `verify-checkout-session`.
- Inoltre, i due test preesistenti in `identity-verification-pickup.test.js` che assumevano il vecchio pagamento simulato sempre riuscito sono stati aggiornati: verificano ora che il gate identità lasci procedere fino al tentativo di creazione della sessione Stripe (testo del pulsante "Reindirizzo al pagamento…"), e — con la rete disabilitata di proposito in quei test — che nessun oggetto risulti mai marcato ritirato senza un pagamento realmente completato.

Suite completa del repository verde (`npm test`, 267/267 nella root; `node --test` nella sottocartella `router/`, 18/18, sito Netlify indipendente).

### Blocco manuale e segnalazioni anti-frode

Gestito in `netlify/functions/save-purchase.js` (regole 1-7, sul singolo acquisto) e `netlify/functions/save-shipment-group.js` (regola 8, sulla spedizione consolidata), ogni volta che un acquisto/gruppo viene salvato/aggiornato. **Settembre 2026**: `isCategoryOutlier()` (regole 3/6) è stata estratta in `netlify/lib/category-stats.js`, condivisa con `category-averages.js` — vedi "Percorso offline — classificazione provvisoria" sopra. Comportamento invariato, solo il codice si è spostato.
...
- **Segnalazioni, mai un blocco automatico** — fino al 1° settembre 2026 esisteva un blocco *automatico* per il pattern "secondo acquisto senza abbonamento": un cliente con già un acquisto registrato e mai stato abbonato, al tentativo di un acquisto aggiuntivo non-abbonato, veniva scritto in `blocklist` e bloccato **permanentemente**. Rimosso dopo un caso reale verificato dal vivo: due acquisti del tutto legittimi (un vero recupero dati) sono stati rifiutati da questa stessa regola. **Nessuna delle otto regole seguenti blocca mai un acquisto o una spedizione** — procedono sempre con lo stesso esito di un acquisto/gruppo qualunque, indipendentemente da quante regole si attivino insieme — ma vengono segnalate direttamente sul record, per revisione manuale dello staff dal CRM.

#### Formato: da valore singolo ad elenco (settembre 2026)

Fino a questa modifica esisteva una sola regola, scritta su un campo a valore singolo: `flaggedReason` (stringa). Con sette regole sull'acquisto (più una sulla spedizione) un record può attivarne più di una insieme, quindi il campo diventa un **elenco**:

- **`flaggedReasons`** — array di stringhe, zero o più valori. Ogni regola che si attiva aggiunge la propria stringa, indipendentemente dalle altre. Se nessuna regola si attiva il campo non è presente sul record (comportamento invariato rispetto a prima).
- **`flaggedAt`** — resta un singolo timestamp ISO, quello dell'ultimo ricalcolo (non uno per regola).
- Ricalcolate **INTERAMENTE ad ogni salvataggio**, esattamente come già faceva la regola originale: mai lette/preservate dal record precedente né dal payload del client (che non le contiene mai). Un acquisto che smette di corrispondere a un pattern (es. il cliente si abbona nel frattempo) semplicemente non viene più segnalato ai salvataggi successivi, senza bisogno di logica di merge.
- Il conteggio/confronto esclude sempre il record dell'item stesso: una risincronizzazione (stesso `id`, es. solo un cambio di stato) non conta mai come un "altro" acquisto per nessuna delle regole.

**Retrocompatibilità** — un record **vecchio** già in store con il campo singolo `flaggedReason` non viene mai toccato da questa modifica finché non viene risincronizzato: resta perfettamente leggibile così com'era (nessun crash, nessuna riscrittura implicita). Se invece viene risincronizzato (es. cambio di stato), viene ricalcolato nel nuovo formato array — stesso comportamento "nessun merge" già in uso per questo campo fin dall'origine, quindi non è un cambio di principio, solo di formato.

**Il CRM va aggiornato** — `flaggedReason`/`flaggedReasons` sono oggi letti e mostrati solo dal repository privato `touchandgo-internal` (non modificabile da qui, vedi la copia di `MANUALE.md` di quel repository per il dettaglio di come). Quel codice si aspetta oggi un valore singolo: un record nuovo con `flaggedReasons` (array, senza più `flaggedReason`) **non causerà un errore lato CRM** (il campo semplicemente non esiste più con quel nome, quindi non verrà mostrato nulla per quel record — nessun crash), ma **la segnalazione smetterà silenziosamente di comparire in interfaccia** per ogni record salvato/risincronizzato dopo questa modifica, finché il CRM non viene aggiornato per leggere ed elencare `flaggedReasons`. Segnalazione esplicita per lo staff di `touchandgo-internal`: questo aggiornamento è necessario, non opzionale, per continuare a vedere le segnalazioni.

#### Le otto regole

1. **Secondo acquisto senza abbonamento** `"Secondo acquisto senza abbonamento"` — [ESISTENTE, logica invariata] un cliente con già almeno un acquisto registrato (email uguale, id diverso) e mai stato abbonato, che effettua un acquisto aggiuntivo non-abbonato.
2. **Acquisti ravvicinati** `"Più acquisti in meno di un'ora"` — più acquisti dalla stessa email con `purchasedAt` (vedi sotto) a meno di un'ora di distanza l'uno dall'altro (finestra simmetrica: prima o dopo). Nuovo campo **`purchasedAt`** (ISO) introdotto su ogni acquisto per questa regola: scritto **solo al primo salvataggio** e sempre preservato da quello già in store alle risincronizzazioni successive (stesso pattern di `deliveryConfirmedAt`/`creditIssued`) — mai fidato se mandato dal client, resta un fatto storico immutabile.
3. **Valore dichiarato anomalo per la categoria** `"Valore dichiarato anomalo per la categoria"` — `itemValue` superiore a **3 volte** la media di `itemValue` calcolata sugli altri acquisti della stessa `category`, **mai** con un campione sotto **5** acquisti in quella categoria.
4. **Stesso indirizzo, email diverse** `"Stesso indirizzo usato da più account"` — stesso `addressLabel` (match esatto, case-sensitive) già usato da un acquisto con un'email diversa (normalizzata) da quella dell'acquisto corrente.
5. **Uso ripetuto di "breakeven"** `"Uso ripetuto di prezzo breakeven"` — `pricingTier === "breakeven"` per l'acquisto corrente, e almeno un altro acquisto della stessa email ha già `pricingTier === "breakeven"`. Coerente con la nota già esistente su questo tier (vedi "Dual pricing e offerte"): pensato come eccezione una tantum (invito monouso o prima spedizione gratuita), non un livello di prezzo permanente per lo stesso cliente.
6. **Peso/dimensioni anomali** `"Peso dichiarato anomalo per la categoria"` — stessa identica logica statistica della regola 3 (stessa funzione condivisa `isCategoryOutlier()`, stessa soglia 3x, stesso campione minimo 5), applicata a `weightKg` invece di `itemValue`. Nessun motivo tecnico per differenziare soglia/campione tra i due campi: la variabilità tra oggetti affini della stessa categoria turistica è paragonabile per un valore in euro e un peso in kg.
7. **Stesso nome, email diverse** `"Stesso nome usato da più account"` — stesso principio della regola 4, applicato a `touristName` invece di `addressLabel` (stessa funzione condivisa `usedByDifferentEmail()`).
8. **Spedizione consolidata con numero insolito di oggetti** `"Numero insolito di oggetti in una spedizione"` — **`netlify/functions/save-shipment-group.js`**, non `save-purchase.js`: si applica al gruppo consolidato nel suo insieme (`group.flaggedReasons`/`group.flaggedAt`, stesso formato array), non al singolo oggetto. Soglia: più di **6** oggetti (7+) in un'unica spedizione — `group.itemIds.length`, mai l'eventuale `itemCount` mandato dal client (stesso principio "mai fidarsi di un valore lato client se calcolabile lato server" già in uso altrove, es. `estimateShown` in `touchandgo-internal`). Soglia scelta abbastanza alta da non scattare su una famiglia numerosa con qualche acquisto extra a testa, abbastanza bassa da intercettare un lotto all'ingrosso mascherato da acquisto turistico.

Le regole 1, 2 e 5 richiedono che l'acquisto corrente abbia un'email (si basano sul confronto con altri acquisti della stessa email); le regole 4 e 7 richiedono anch'esse un'email propria (per poter dire "diversa dalla mia"); le regole 3, 6 e 8 non dipendono dall'email e si applicano sempre.

**Verifica**: 22 nuovi test (oltre ai 5 preesistenti, aggiornati al nuovo formato array).
- `netlify/functions/__tests__/save-purchase.block-flag.test.js` (5, aggiornato) — regola 1 verificata nel nuovo formato array (`flaggedReasons.includes(...)`, non più un confronto sull'intero elenco, perché nei test gli acquisti ravvicinati fanno scattare *legittimamente* anche la regola 2); blocco manuale invariato; risincronizzazione non si auto-flagga.
- `netlify/functions/__tests__/save-purchase.fraud-rules.test.js` (18, nuovo) — ciascuna delle regole 2-7 verificata isolatamente nel proprio scenario; le due regole statistiche (3/6) verificate sia sopra sia sotto soglia, e verificate non scattare mai con un campione insufficiente; un acquisto che attiva **sei** regole contemporaneamente (1, 2, 3, 6, 4, 7 insieme) le contiene tutte nell'elenco, con risposta sempre `200`; nessuna regola attivata → campo assente (comportamento invariato); un record vecchio nel formato a stringa singola non viene mai riscritto se non risincronizzato.
- `netlify/functions/__tests__/save-shipment-group.fraud-rule.test.js` (4, nuovo) — regola 8 sopra soglia (7 oggetti) → flag, sempre `200`, gruppo comunque salvato, prezzo invariato; esattamente alla soglia (6) → nessun flag; caso comune (pochi oggetti) → nessun flag; un `itemCount` mandato dal client viene ignorato, il conteggio reale è sempre `itemIds.length`.

Suite completa del repository verde (`npm test`, 300/300 nella root; `node --test` nella sottocartella `router/`, 18/18, sito Netlify indipendente).
...
| `category-averages.js` | Peso/dimensioni medi reali per ciascuna delle 10 categorie (con fallback dichiarato sotto una soglia minima di campione) — vedi "Percorso offline — classificazione provvisoria" in "App turista". Unica fonte per il prezzo provvisorio offline, mai interrogata al momento del bisogno. |
...
**Aggiunta (settembre 2026)**: nella sezione "IL PROGETTO", dopo "Perché i numeri reggono", una nuova sottosezione "Come raggiungiamo gli utilizzatori" spiega il canale di acquisizione (rete di tour operator/agenzie/negozi partner) e il suo legame diretto col CAC reale già citato più sopra nella stessa sezione — diff minimo, solo questa aggiunta, tag bilanciati verificati.

**Correzione + aggiunta (settembre 2026)**: rimosso il testo obsoleto sulla password del CRM (rimossa il 13 settembre 2026, ora mostrabile a investitori senza condividerla) da tutte e 4 le occorrenze in questa pagina — il link resta identico, solo l'etichetta cambia. Aggiunte anche le card per i due dispositivi mancanti nel blocco principale dei link: Gestionale (`touchandgo-gestionale.netlify.app`, partner B2B — agenzie/tour operator/negozi) ed E-shop (`touchandgo-eshop.netlify.app/site/index.html`, vetrina prodotti per partner) — URL verificati via API Netlify, non assunti.

**Griglia "La suite in un colpo d'occhio" — da 6 a 8 schede (settembre 2026)**: la card "04 Gestionale (CRM)" era ambigua da quando esiste un vero dispositivo Gestionale B2B separato — rinominata semplicemente "CRM" (stessa icona/link/descrizione, nessun altro cambiamento). Aggiunte due nuove card nella griglia `.hub-grid`: "Gestionale B2B" (riga superiore, dopo "Area partner") ed "E-shop" (riga inferiore, dopo "Report di rendimento") — entrambe con `target="_blank" rel="noopener"` (nessun link esterno preesistente in questo file da cui dedurre un pattern diverso), e un simbolo "↗" al posto del numero di sezione nell'angolo della card (`hc-num`) perché, a differenza delle altre 6, non aprono un'ancora interna alla guida ma un sito esterno — usare un numero tipo "05"/"06" avrebbe suggerito una sezione di dettaglio inesistente (la sezione "05" reale della guida è tutt'altro, "Il percorso consigliato in 10 minuti").

**Disposizione scelta**: griglia estesa da 3 a 4 colonne (`grid-template-columns:repeat(4,1fr)`, `max-width` da 820px a 900px), card centrale "Touch&Go" adattata a spannare le 4 colonne (`grid-column:1/5`) — stessa struttura concettuale di prima (riga superiore → fulcro centrale → riga inferiore), solo con una card in più per lato: 4 sopra, 4 sotto, il fulcro resta esattamente al centro. Gestionale B2B ed E-shop finiscono così nella stessa colonna (la quarta), una sopra l'altra — un accostamento visivo sensato, dato che sono gli unici due dispositivi standalone rivolti ai partner esterni al prototipo principale.

**Correzione consequenziale**: rinominare le schede da 6 a 8 rendeva sbagliati due testi che citavano il conteggio esplicito — "Sei porte d'accesso, un solo prodotto" nel sottotitolo di copertina (→ "Otto porte d'accesso...") e "↑ Torna alle sei schede" in tutti i link di ritorno a fine sezione, 6 occorrenze (→ "↑ Torna alle otto schede"). Non sono sezioni diverse della guida in senso stretto — sono boilerplate che dipende direttamente dal numero di card della griglia appena modificata — ma lasciarli avrebbe reintrodotto esattamente il tipo di incoerenza visibile a un investitore che questa modifica doveva evitare.

**Verifica visiva effettuata**: screenshot reali via Playwright/Chromium (headless, aperti da `file://` sul file modificato) della sola `.hub-section`, viewport desktop (1280×900) e mobile (390×844), più uno screenshot della `.cover` per verificare il testo aggiornato. Desktop: 4+4 card ben allineate, linee di connessione verso il centro presenti e centrate su ogni card come nelle 6 originali, nessuna sovrapposizione, testo leggibile su tutte le card nuove. Mobile: tutte le 8 card impilate correttamente in colonna singola (`.hub-center` in cima via `order:-1`, invariato), nessun overflow orizzontale. Unica osservazione (preesistente, non causata da questa modifica, non toccata): il bottone flottante "Chiedi a Touch&Go" (`#tg-chat-toggle`, `position:fixed;bottom:22px;right:22px`) copre parzialmente il testo della card "Gestionale B2B" nello screenshot mobile iniziale (senza scroll) — succede perché quella card capita in quella posizione verticale del viewport, non per un problema di layout della griglia: lo stesso bottone coprirebbe qualunque contenuto si trovasse in quella posizione, indipendentemente da questa modifica.

**Link esterni sempre in una nuova scheda + URL corretti verso le tab del CRM (settembre 2026)**: due correzioni insieme, entrambe verificate esaustivamente su tutto il file.

- **`target="_blank" rel="noopener"` su ogni pulsante `.btn`**: prima di questa modifica solo le 2 card esterne dell'hub (Gestionale B2B, E-shop — vedi sopra) lo avevano; i 13 pulsanti `.btn` nei `.linkcard` (6 nella sezione "ACCESSO RAPIDO" più 7 sparsi nelle sezioni di dettaglio) ne erano privi, incluse le voci che restano tecnicamente sullo stesso dominio (`touchandgo-suite.netlify.app`) ma portano comunque a una pagina diversa da questa guida — stesso trattamento per coerenza, nessuna eccezione. Verificato con una ricerca esaustiva di `class="btn"` dopo la modifica: **13/13** con l'attributo.
- **URL corretti verso le tab del CRM**: da quando il CRM (`touchandgo-internal`) supporta l'apertura diretta su una tab specifica via `?tab=slug`, due `linkcard` la cui etichetta promette una tab precisa puntavano ancora genericamente a `admin.html` (aprendo sempre Panoramica): "CRM — tab Recensioni, per moderare" (sezione `s-broadcasting`) → ora `admin.html?tab=recensioni`; "CRM — tab Sistema" (sezione `s-report`) → ora `admin.html?tab=sistema`. Aggiornato anche il testo `.lc-url` mostrato accanto al pulsante, per restare coerente con l'`href` reale. Cercate esplicitamente altre occorrenze dello stesso problema (etichetta che promette una tab specifica ma URL generico): **nessun'altra trovata** — le due card semplicemente etichettate "CRM" (senza menzione di una tab) puntano correttamente alla pagina generica, dato che non promettono nulla di più specifico.
...
## Invito vetrina online del partner + "Per te" dopo l'acquisto (settembre 2026)

Dopo un acquisto completato (pagamento Stripe confermato — `ShippedScreen()`, `dist/assets/app.js`), un invito **solo visivo** — mai un redirect automatico — verso la vetrina online del negozio che ha generato/riferito l'acquisto, più un secondo invito **sempre presente** verso una pagina cross-partner personalizzata sullo storico d'acquisto del turista. Nessuna modifica alla logica di acquisto/pagamento esistente: il calcolo è additivo, eseguito subito prima che `finalizeShippedGroups()` svuoti `state.pendingItems`.

### Dove si trova

`ShippedScreen()` è la schermata di conferma acquisto — "✓ Ordine confermato e pagato" — mostrata dopo il ritorno da Stripe Checkout con pagamento verificato server-side (non `QueuedScreen()`, che è invece lo stato intermedio "🕓 QR generato — in sospeso": il ritiro non è ancora partito, l'acquisto non è "completato"). `finalizeShippedGroups()` (chiamata solo dopo la verifica reale del pagamento) calcola `state.shippedPartnerInvites` dagli item ancora presenti in `state.pendingItems` **prima** che l'array venga svuotato — è l'ultimo punto in cui gli item spediti sono disponibili con i loro campi `partnerCode`/`generatedByPartnerCode` originali (i gruppi semplificati salvati in `state.shippedGroups` non li conservano).

### Come si sceglie il link della vetrina

`partnerEshopInvitesForItems(items)` estrae, dedupli­candoli, i codici partner (`generatedByPartnerCode` ha priorità su `partnerCode` — identifica chi ha davvero generato la spedizione, mentre `partnerCode` resta riservato al meccanismo di commissione sugli sconti, invariato) e li confronta con **`PARTNER_ESHOP_INFO`**, una mappa hardcoded (oggi un solo negozio: `NDP924` → "Negozio Demo", `https://touchandgo-eshop.netlify.app/site/index.html`) — stesso principio isolato/hardcoded già usato per `DEMO_PARTNER_CODE` nell'altro repository (`touchandgo-eshop`): nessun lookup dinamico finché esiste un solo negozio con una vetrina pubblicata. Un acquisto self-service ordinario (nessun campo partner valorizzato, o un partner senza vetrina pubblicata) produce un elenco vuoto — nessun invito verso una vetrina che non esiste.

**Perché non un vero lookup lato server, oggi**: recuperare il *nome* di un partner qualunque richiederebbe una function pubblica dedicata (lo store `partners` non ha oggi un endpoint di sola lettura sicuro per questo). `netlify/functions/partner-stats.js` esiste già, ma **non va mai riusato per questo**: richiede il codice partner come credenziale ed espone dati finanziari reali (vendite, commissioni) — chiamarlo da questo invito esporrebbe quei dati a qualunque turista che conosce il codice partner tramite un proprio acquisto. Quando arriveranno altri partner con vetrina pubblicata, `PARTNER_ESHOP_INFO` andrà sostituita da un vero lookup pubblico (nome + URL soltanto).

### Il link "Per te" — sempre presente

`perTeUrl(touristEmail)` costruisce sempre `https://touchandgo-eshop.netlify.app/site/per-te.html?email=<touristEmail urlencoded>`, indipendentemente dal partner — anche per un acquisto self-service. La pagina non esiste ancora in questo momento (costruita separatamente in `touchandgo-eshop`): il link è comunque corretto e punterà a una pagina reale a breve.

### Markup e sicurezza

`partnerEshopInviteHtml(invites, touristEmail)` costruisce il markup finale (classe `.eshop-invite`, riusa `.btn-secondary` con una piccola regola CSS aggiuntiva — `display:block` — per funzionare su un `<a>` invece che solo su `<button>`, nessun colore/font nuovo). `escapeHtml()` applicato al nome del negozio e a ogni URL (vincolo esplicito) — anche se `PARTNER_ESHOP_INFO` è oggi hardcoded e "fidata", la funzione deve restare sicura quando il nome arriverà da un vero lookup lato server in futuro.

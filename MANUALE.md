...
### Bottone "Assistenza" persistente (sito e app)

A differenza dell'assistente conversazionale sopra (solo modalità Turista, risponde con l'AI), questo bottone raccoglie una **segnalazione libera per lo staff** — presente su **ogni** pagina del sito marketing e **ogni** schermata dell'app, in entrambe le modalità Turista e Partner.

- **Backend**: azione `submit-support-request` in `netlify/functions/sync.js` (stesso rate limit — 20 richieste/60 minuti per IP — delle altre azioni pubbliche del file). Riceve `{ message, contactEmail (opzionale), context, userAgent (opzionale) }`; valida solo che `message` non sia vuoto e non superi 2000 caratteri. Salva un record con `status: "nuovo"` e timestamp nel nuovo store Blobs **`support-requests`** — nessuna UI di gestione ancora nel CRM, i record restano consultabili solo direttamente dallo store.
- **`context`**: stringa libera che identifica da dove arriva la segnalazione, per orientare lo staff senza dover richiedere di nuovo la posizione all'utente.
  - Sito marketing: nome pagina, es. `"sito - home"`, `"sito - come funziona"`, `"sito - privacy"`, `"sito - termini"`.
  - App: schermata e lingua correnti, es. `"app - result (it)"`.
- **Sul sito** (`dist/site/index.html`, `come-funziona.html`, `privacy.html`, `termini.html`): bottone fisso in basso a destra (`.support-fab`), sempre visibile durante lo scroll, apre un piccolo overlay con textarea + email opzionale. **Nessun file JS/CSS condiviso tra le pagine** (nessun framework/build step in questo progetto, vedi Panoramica) — lo stesso blocco HTML/CSS/JS è duplicato identico nei 4 file, con l'unica riga che cambia legittimamente da un file all'altro `var SUPPORT_CONTEXT = "..."`. Attenzione a questo quando si modifica il widget: va aggiornato in tutti e 4 i file allo stesso modo, altrimenti divergono silenziosamente.
  - Nota estetica minore: su `come-funziona.html`, il contenuto vive dentro un riquadro centrato in stile "cornice telefono" (`#stage`, max 420px) — il bottone, fisso rispetto alla finestra del browser e non a `#stage`, su schermi larghi appare quindi ancorato all'angolo della finestra reale, non a quello della cornice. Scelta deliberata per mantenere il blocco identico nei 4 file invece di differenziarlo per questa pagina.
- **Nell'app** (`dist/assets/app.js`): stesso bottone dentro `Header()`, accanto a "💬 Chiedi a Touch&Go" — usa lo stesso identico meccanismo di persistenza (nessuna logica di visibilità nuova), quindi non compare durante l'onboarding (come tutto `Header()`) e compare sempre altrove, in entrambe le modalità. Al tap apre `SupportRequestModal()`, un overlay costruito sullo stesso pattern di `AssistantChatModal()` (stesse classi CSS `.assistant-chat-*` per l'overlay/il modale/l'intestazione, campi propri `.support-field`/`.support-modal-textarea`/`.support-modal-input` per il form). Etichette tradotte nello stesso dizionario `I18N`/`t()` — il testo scritto dall'utente non viene tradotto.
...
`sync.js` non ha un rate limit generale (le sue azioni richiedono id/codici già noti al chiamante), tranne due azioni raggiungibili da chiunque senza credenziali: `get-purchases-by-email` (vedi "Sincronizzazione dello storico acquisti tra dispositivi" più sopra), che accetta un'email libera, e `submit-support-request` (vedi "Bottone 'Assistenza' persistente" più sopra) — entrambe applicano lo stesso schema 20/60 minuti per IP.

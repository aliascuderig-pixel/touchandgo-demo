...
9. **Conclusione soggiorno** (`ConcludeScreen`) — quando il turista ha finito di fare acquisti, consolida tutti gli acquisti "in sospeso" per destinazione in un unico ordine di ritiro (`ShippedScreen`) — qui gli acquisti passano storicamente a "ritirato" in un colpo solo (percorso legacy, oggi affiancato dal flusso più granulare a 4 stati descritto sotto). **È anche qui, e solo qui, che avviene il calcolo finale e il momento del pagamento** — vedi "Il momento del pagamento" più sotto.

### Il momento del pagamento

Oggi Touch&Go **non ha un pagamento reale integrato**: nessun collegamento a Stripe o ad altro PSP, il pagamento è simulato (un semplice `setTimeout` prima di passare alla schermata di conferma). Questo non significa però che il *momento* in cui il pagamento (anche solo simulato) avviene sia arbitrario: è costruito fin da ora nel punto giusto, così un'integrazione futura di un pagamento vero si aggancia lì senza dover ristrutturare il flusso.

- **Durante lo shopping (`ResultScreen`, subito dopo la classificazione di ogni oggetto)**: il prezzo mostrato è **sempre e solo una stima per la spedizione di questo singolo oggetto** — badge "STIMA · spedizione singola" e nota esplicita ("nessun addebito ora") sotto ogni preventivo, qualunque sia l'offerta mostrata (pieno/abbonamento, breakeven, prima spedizione gratuita). Nessun addebito, nessuna conferma di pagamento avviene qui: è un'anteprima, non un impegno.
- **Solo alla "Conferma e invia ordine di ritiro consolidato" (`ConcludeScreen`)**: qui avviene sia il calcolo finale — per destinazione, la somma dei prezzi degli oggetti consolidati verso quella destinazione (`totalsByDest` in `ConcludeScreen`; se un solo oggetto è pronto per quella destinazione il "totale del gruppo" coincide, per costruzione, col suo prezzo singolo) — sia, concettualmente, **il pagamento**: il pulsante di conferma mostra esplicitamente il totale complessivo da pagare ("Conferma e paga €X — ordine di ritiro consolidato →"), preceduto da un riepilogo con il totale per ogni destinazione e il totale complessivo. Il codice tratta esplicitamente questo istante come "il turista sta confermando e pagando" e non solo "sta notificando un ritiro" — sia nel testo a schermo sia in un commento dedicato sopra `confirmBtn` in `ConcludeScreen`.
- **Punto di integrazione per un pagamento reale futuro**: segnato da un commento esplicito in `dist/assets/app.js`, subito sopra la definizione di `confirmBtn` in `ConcludeScreen` — quando arriverà un vero PSP (es. Stripe), la chiamata va agganciata lì, **prima** del blocco che marca gli oggetti come `"ritirato"` (`it.status = "ritirato"`) e li sincronizza col CRM (`syncPurchaseToCRM`), non dopo: oggi l'ordine viene sempre "confermato" incondizionatamente dal `setTimeout`, ma un pagamento reale potrebbe fallire, e in quel caso gli oggetti non andrebbero comunque marcati come ritirati.
- `ShippedScreen` (schermata successiva) rispecchia lo stesso principio: titolo "Ordine confermato e pagato", una riga che riepiloga il totale pagato (somma dei totali per destinazione) esplicitamente marcata come "registrato (simulato in questo prototipo)", e la nota prototipo aggiornata per menzionare, oltre al corriere, anche l'assenza di una richiesta reale a un istituto di pagamento.

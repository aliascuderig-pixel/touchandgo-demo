...
Tutta l'esperienza vive in `dist/assets/app.js`, un'unica applicazione a schermate (`state.screen`) senza router — ogni funzione tipo `CaptureViewfinderScreen()`, `ResultScreen()` ecc. costruisce il DOM della schermata corrente e viene richiamata da `render()`.
...
1. **Cover** — schermata di apertura, mostra il punto di ritiro rilevato (GPS o stima da rete). Ospita anche il `Footer()` (Dashboard/Storico/Reset/Termini/Privacy) — vedi "Mirino immersivo" più sotto per il perché.
2. **Home** (`CaptureViewfinderScreen`) — mirino immersivo a schermo intero, attivo fin dall'arrivo sulla schermata: foto dell'oggetto (fotocamera o galleria) oppure, dal link dedicato, descrizione testuale/vocale libera. Il campo facoltativo "Hai un codice invito?" (offerta breakeven, vedi sotto) resta qui, tra i controlli overlay in basso. Vedi "Mirino immersivo, schermata di acquisizione (TOU-21)" più sotto.
...
### Mirino immersivo, schermata di acquisizione (TOU-21, Opzione B)

La schermata **Home** (`CaptureViewfinderScreen()` in `app.js`, chiamata da `render()` invece del vecchio `HomeScreen()`) non è più un riquadro statico "Fotografa l'oggetto" con un pulsante "Apri la fotocamera" che, al tap, apriva un mirino separato: il mirino **è** la schermata stessa fin dal primo istante. Arrivando su Home (tap su "Tocca per iniziare" in Cover, o ritorno da un'altra schermata) il feed live della fotocamera è già lo sfondo a schermo intero, senza alcuno stato intermedio da toccare per attivarlo. Giuseppe aveva scartato due varianti precedenti (differivano solo nel colore) chiarendo che voleva un cambio di struttura, non estetico: ha scelto tra tre mockup l'"Opzione B", implementata qui in versione funzionante (non più solo un mockup).

- **Niente chrome normale dell'app su questa schermata**: `render()` intercetta `state.screen === "home"` (modalità turista) prima di appendere `Header()`/`Footer()`, esattamente come già faceva per `OnboardingScreen` — mode-toggle Turista/Partner, pulsante reset, "Chiedi a Touch&Go" e i link di `Footer()` non compaiono qui. Restano tutti raggiungibili tornando a **Cover** col pulsante "←" in alto a sinistra del mirino: `Footer()` (Dashboard/Storico/Reset/Termini/Privacy, prima in fondo a Home) è stato spostato lì per questo motivo — altrimenti sarebbe diventato irraggiungibile.
- **Overlay minimale sopra il feed, mai una barra opaca**: barra superiore con pulsante indietro, wordmark "Touch&Go" e selettore lingua IT/EN come pillole semi-trasparenti con blur (`.vf-pill`, stesso principio già usato per `.ob-lang-group`/`.ob-skip-btn` nell'onboarding); eventuali banner contestuali (errore, promemoria posizione GPS, acquisti in sospeso — prima nel corpo di `HomeScreen`, ora overlay compatti sotto la barra superiore, `ViewfinderBanners()`, nessuna funzionalità persa); cornice ad angoli dorati **riusata identica, non ridisegnata** (`.viewfinder-frame`/`.vf-corner`, già confermata funzionante su dispositivo reale) con un testo guida sopra ("Inquadra l'oggetto da spedire"); in basso un cluster overlay su scrim scuro sfumato — badge "🛡️ Copertura inclusa · Tracciato via WhatsApp" (stesso testo/informazione di `TrustRow`, ora rimossa perché ridondante: le due voci più rilevanti vivono nel badge, quella sulla dogana automatica non era richiesta qui), riga controlli (galleria a sinistra, scatto grande al centro, cambio fotocamera a destra), link "Descrivilo a voce" e, più in piccolo, "Hai un codice invito?".
- **Un solo pulsante di scatto in tutto il flusso Home**: `.vf-home-shutter`, il cerchio bianco all'interno del mirino — è anche l'unico "tap per iniziare" (nessun secondo tap su un'icona placeholder). Questo risolve strutturalmente **TOU-20**: lo stato "icona pre-tap" (prima un'iride SVG animata, `apertureIconMarkup()`, rimossa insieme al vecchio `HomeScreen`) non esiste più, perché non c'è più un riquadro intermedio da mostrare prima del mirino. `playShutterSound()` resta agganciato esclusivamente a questo pulsante (stesso `AudioContext` condiviso con `resume()` esplicito se sospeso, verificato ancora presente e funzionante — vedi funzione in `app.js`).
- **Cambio fotocamera** (`flipHomeViewfinderCamera()`): alterna `facingMode` tra `"environment"` e `"user"` e riapre lo stream — scelto rispetto a un torch/flash reale perché quest'ultimo (`MediaStreamTrack` con capability `torch`) non è supportato in modo affidabile su Safari iOS, la piattaforma più diffusa tra i turisti target.
- **Galleria**: icona a sinistra nel cluster controlli, stesso `<input type="file" accept="image/*">` nascosto di sempre (`handleFile`) — invariato nella logica, solo spostato dal vecchio riquadro "Scegli dalla galleria" a un'icona overlay.
- **Percorso "descrivilo a voce"** (`HomeDescribeScreen()`): stesso percorso di descrizione manuale già esistente (`handleDescribe`), raggiunto dal link in basso nel mirino — ferma lo stream della fotocamera (non ha senso un feed live su una schermata pensata per chi non fotografa) e mostra un campo di testo con la stessa dettatura vocale riusabile già presente altrove in app (`addVoiceButton()`), non ancora applicata a questo campo prima di questa modifica.
- **Ciclo di vita dello stream camera**: `homeViewfinderStream` (module-level) viene riusato tra un render e l'altro (nessun nuovo prompt di permesso, nessun flicker) e chiuso esplicitamente (`stopHomeViewfinder()`) ogni volta che si esce dalla schermata Home in modalità fotocamera — cambio schermata, passaggio a modalità Partner, switch a "descrivilo a voce", scatto completato — perché `getUserMedia` non si ferma da solo quando il `<video>` a cui è agganciato viene rimosso dal DOM (`app.innerHTML = ""` a ogni `render()`). Il mirino riparte sempre in modalità fotocamera quando si **entra** in Home da un'altra schermata, anche se l'ultima volta era stato lasciato in modalità "descrivilo a voce".
- **Cattura**: al tap sullo scatto, il fotogramma corrente del video viene disegnato su un `<canvas>` e convertito in JPEG (`canvas.toDataURL`) — stesso formato (data URL) che l'app riceveva già da `FileReader` sui file dell'input nativo, quindi tutto il codice a valle (classificazione AI) resta invariato.
- **Fallback automatico**: se `getUserMedia` non è disponibile o il permesso fotocamera viene negato, il mirino mostra comunque l'intera struttura overlay su uno sfondo scuro fisso invece del feed live; lo stesso pulsante di scatto, in questo stato, apre l'`<input type="file" capture="environment">` nativo rimasto presente in pagina — coerente con "un solo pulsante di scatto che è anche l'unico tap per iniziare" anche quando il mirino custom non è disponibile. Un avviso discreto a schermo, "Fotocamera del dispositivo in uso" (`.camera-fallback-toast`), conferma che il fallback è scattato davvero; `console.warn` registra il motivo reale per la diagnosi da remoto.

**Verifica imballo** (`PackageCheckScreen`, punto 7 del flusso sopra) **non è cambiata**: resta un riquadro "Fotografa l'imballo pronto" con emoji 📷 che, al tap, apre il mirino come overlay (`openCameraViewfinder()`, la stessa funzione di sempre — cornice ad angoli dorati, scatto, suono otturatore, fallback identici) — fuori dallo scope di questa modifica, che riguarda solo la schermata di acquisizione dell'oggetto (Home).
...
**Cosa copre la FASE 1** (architettura + percorso principale d'acquisto): CoverScreen, CaptureViewfinderScreen/HomeDescribeScreen (schermata Home, vedi "Mirino immersivo"), DestinationScreen, AnalyzingScreen, ResultScreen, PackageCheckScreen, IdentifyScreen, DocumentsScreen, più i componenti condivisi che vi compaiono (Header, Footer, AssistantAvatar, PickupField/DestinationField/GuestDestinationField, PartnerDiscountField, AddressFormFields).

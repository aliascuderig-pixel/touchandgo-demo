...
- **Test**: `netlify/functions/__tests__/assistant.test.js` — copre le tre modalità: il system prompt costruito per ciascuna (diretto via `buildSystemPrompt()` esportato ed end-to-end tramite l'handler con la chiamata Anthropic mockata), la resistenza a un tentativo di prompt injection su "spiega_la_suite", e che un `mode` sconosciuto o assente ricada correttamente su "domanda" — invariato dopo l'aggiunta della terza modalità.

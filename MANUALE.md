...
### Gap in `privacy.html`: rilevamento firma via AI non dichiarato (corretto)

Una revisione di sicurezza/privacy ha trovato che `dist/site/privacy.html` dichiarava correttamente l'invio ad Anthropic delle foto degli oggetti per la classificazione doganale (punti 2 e 5), ma **non menzionava da nessuna parte** lo stesso identico meccanismo (`classify()`, stesso endpoint `netlify/functions/classify.js`) usato da `detectSignature()` per rilevare la firma sul documento di riconoscimento caricato in `IdentifyScreen` — nonostante un documento di riconoscimento sia un dato ben più sensibile di una foto di un oggetto acquistato.

**Verificato nel codice, non dato per assunto, prima di scrivere qualunque correzione**: `grep -rn "idDocument" netlify/functions/` restituisce **zero corrispondenze** — nessuna Netlify Function di questo repository riceve mai il documento. `detectSignature()` lo invia solo (in transito, via `classify()`) a `netlify/functions/classify.js`, un puro proxy verso l'API Anthropic che non lo persiste in alcuno store Blobs (l'unico store toccato da quella function è `rate-limits`, un contatore anti-abuso, più una lettura di sola consultazione da `customs-reference` — nessuno dei due contiene il documento). Il documento viene salvato **solo** in `localStorage` (`saveProfile()`/`loadProfile()`, chiave `tg_profile`) insieme a nome/indirizzi/cronologia — mai incluso nei payload inviati a `save-purchase.js`/`sync.js` (verificato leggendo `syncPurchaseToCRM()` e ogni altra chiamata `fetch("/.netlify/functions/...")` in `app.js`: nessuna include `state.idDocument` o `tg_profile`). La dichiarazione già esistente della nota positiva ("il documento resta solo sul dispositivo") era quindi vera — il gap era solo l'omissione del trattamento AI transitorio dalla privacy policy, non un problema di codice.

**Corretto in `privacy.html`**: una riga nella tabella del punto 2 ("Documento di riconoscimento (facoltativo)"), una frase esplicita nel punto 3 che il documento resta solo sul dispositivo esattamente come nome/indirizzi/cronologia, e una riga dedicata nella tabella dei fornitori terzi al punto 5 (Anthropic, stesso fornitore già presente per le foto oggetti, riga separata perché il dato ricevuto e lo scopo sono diversi). Nessuna modifica alla logica applicativa — solo testo/contenuto legale, per una revisione ulteriore da parte di chi segue la parte legale prima del lancio commerciale (vedi nota in cima al documento stesso).

...
- **Fasi successive**: il CRM interno (repository `touchandgo-internal`) ha ricevuto lo stesso modello di design token — PR aperta, non ancora mergiata (vedi Linear TOU-21). Sito pubblico marketing: non ancora fatto.
- **Nota — non confondere con "Mirino immersivo" (Opzione B)**: dopo aver visto queste due varianti (solo palette/font, nessuna differenza strutturale), Giuseppe ha chiesto un cambio *strutturale/di interazione* per la schermata di acquisizione turista, non solo colore — vedi "Mirino fotocamera con rumore otturatore (TOU-20 / TOU-21 Opzione B)" più sotto. È un lavoro distinto, applicato qui in Home, pensato per essere esteso concettualmente (non copiato 1:1, ogni superficie va reinterpretata) alle altre superfici della suite con prompt separati futuri, solo dopo conferma di Giuseppe sul test reale di questa schermata.
...
2. **Home** (`HomeScreen`) — il mirino immersivo si apre da solo all'arrivo (vedi "Mirino immersivo" più sotto): fotografa l'oggetto, scegli dalla galleria o descrivilo a voce/testo. Qui compare anche il campo facoltativo "Hai un codice invito?" (offerta breakeven, vedi sotto).
...
### Mirino fotocamera con rumore otturatore (TOU-20 / TOU-21 Opzione B)
...
- **UI a mirino, condivisa da entrambe le schermate**: overlay a schermo intero (`.viewfinder-overlay`) con anteprima video live e una cornice ad angoli in stile reflex (`.viewfinder-frame`, quattro `.vf-corner` dorati) — **non ridisegnata, riusata identica** tra le due varianti sotto. **Confermato funzionante su dispositivo reale da Giuseppe.**
- **Cattura**: al tap sul pulsante di scatto (`.vf-shutter`, unico in tutto il flusso, con lo stesso rumore otturatore in entrambe le varianti — vedi sotto), il fotogramma corrente del video viene disegnato su un `<canvas>` e convertito in JPEG (`canvas.toDataURL`) — stesso formato (data URL) che l'app riceveva già da `FileReader` sui file dell'input nativo, quindi tutto il codice a valle (classificazione AI, verifica imballo) resta invariato.
...
- **Rumore di scatto**: `playShutterSound()` sintetizza via Web Audio API due brevi impulsi di rumore filtrato (apertura/chiusura otturatore) — non un file audio esterno: nessun asset da scaricare né da licenziare, funziona anche offline nella PWA. **Bug corretto (TOU-20)**: creava un nuovo `AudioContext` ad ogni scatto senza mai controllarne lo stato — su Safari/Chrome mobile un `AudioContext` può nascere `"suspended"` anche dentro un gesto utente reale, e senza un `resume()` esplicito i suoni programmati non si sentono e non sollevano alcun errore (esattamente il sintomo riportato: "nessun suono, neanche l'ombra", nessun errore in console). Ora un solo `AudioContext` condiviso e riusato (creato al primo scatto), con `resume()` esplicito quando risulta sospeso prima di programmare il suono; eventuali errori finiscono in `console.warn`, non più inghiottiti in silenzio. Con l'arrivo del mirino immersivo su Home (sotto), il rumore resta legato **esclusivamente** a questo pulsante — non esiste più alcun altro punto del flusso che lo riproduce.

#### Due varianti di chrome, una funzione sola (`openCameraViewfinder(onCapture, fallbackInput, opts)`)

`opts` è opzionale — solo `beginHomeCapture()` (Home) lo passa. Senza `opts`, `PackageCheckScreen` continua a ricevere esattamente il mirino "minimo" di sempre: bottone ✕ in alto a destra (`.vf-close`), cornice, scatto — **non toccato** da TOU-21 Opzione B.

**Home — mirino immersivo (TOU-21 Opzione B, sostituisce l'"icona placeholder pre-tap" TOU-20)**

Correzione di scope esplicita di Giuseppe dopo aver visto le prime due varianti (solo colore, non struttura): niente più primo passo da toccare prima di aprire la fotocamera. Il mirino **è** la schermata Home fin dall'arrivo:

- **Apertura automatica**: `render()`, ogni volta che si "arriva" su Home in modalità turista (modalità+schermata cambiate rispetto all'ultimo render — quindi non ad ogni re-render mentre si resta su Home, es. cambio lingua o promemoria posizione chiuso), chiama `beginHomeCapture()` — l'utente si trova già nel mirino attivo, senza dover toccare nulla. Il vecchio riquadro placeholder "Fotografa l'oggetto" con l'icona a iride (`apertureIconMarkup()`, ancora presente in `style.css`/`app.js`) **non è stato rimosso**: resta come schermata di riserva, visibile solo se il mirino non si apre da solo (mai stato aperto con successo, es. permesso negato — in quel caso `openCameraViewfinder` ricade comunque sul fallback nativo) o se l'utente lo chiude esplicitamente senza scattare — un tap sul riquadro lo riapre. Non anima più l'iride né riproduce un suono al tap (comportamento TOU-20 ormai superato dalla nuova struttura): il suono resta legato solo al vero pulsante di scatto.
- **Chrome sopra il feed live**, tutta `position:absolute` rispetto a `.viewfinder-overlay` (stesso approccio di `.vf-close`/`.vf-shutter`, non flex — evita conflitti col centraggio di testo guida/cornice) — pillole semi-trasparenti su scrim sfumati (`.vf-scrim-top`/`.vf-scrim-bottom`), stesso linguaggio visivo già stabilito per l'onboarding (`.ob-lang-group`/`.ob-skip-btn`/`.ob-wordmark`):
  - **Barra superiore** (`.vf-topbar`): pulsante indietro (`.vf-back`, chiude il mirino e rivela la Home di riserva), wordmark "Touch&Go" (`.vf-wordmark`), selettore lingua IT/EN (`.vf-lang-group`/`.vf-lang-btn`) — cambiare lingua qui chiama `setLang()` (che re-renderizza la Home sottostante) e poi aggiorna sul posto i testi del mirino stesso (`refreshChrome()`, interno a `openCameraViewfinder`) **senza chiudere/riaprire lo stream della fotocamera**.
  - **Testo guida** (`.vf-guide`) sopra la cornice: "Inquadra l'oggetto da spedire" / "Frame the item you want to ship".
  - **Badge fiducia** (`.vf-badge`), sopra la riga controlli: riusa le stesse chiavi `trust_coverage`/`trust_tracked` già usate in `TrustRow()`, non un testo duplicato — "Copertura inclusa · Tracciato via WhatsApp".
  - **Riga controlli**: galleria a sinistra (`.vf-gallery`, apre un file input dedicato con lo stesso `handleFile()` della gallery-card esistente), scatto al centro (`.vf-shutter`, invariato), flash/torcia a destra **solo se la traccia video lo supporta davvero** (`track.getCapabilities().torch` — in pratica Chrome/Android; Safari/iOS non lo espone) — se non disponibile il controllo non compare affatto: meglio una riga più essenziale che un bottone che non farebbe nulla. Nessun secondo accesso alla galleria duplicato al suo posto.
  - **Link "Non puoi fotografarlo? Descrivilo a voce"** (`.vf-describe-link`) sotto i controlli: chiude il mirino e porta il focus sul campo di descrizione testuale già esistente in Home (`#home-describe-input`, con scroll automatico) — **non aggiunge dettatura vocale nuova**: il testo del link riprende quello del mockup ricevuto, ma la lettura scelta (dichiarata, non decisa in silenzio) è che rimanda al percorso alternativo testuale già presente, non a una funzione voce non ancora esistente — un microfono su quel campo avrebbe anche creato una sovrapposizione visiva con la freccia di invio "→" già lì.

**`PackageCheckScreen` — invariato**: nessuna chrome aggiuntiva, nessuna icona placeholder mai avuta, nessuna modifica di comportamento. L'unico effetto collaterale (dichiarato, non un cambio di scope): l'aria-label del pulsante di scatto condiviso ora passa da `t("capture_shutter_aria")` invece di un letterale sempre in italiano — stessa parola di default ("Scatta foto"), ma tradotta correttamente in inglese per chi usa l'app in EN, cosa che prima non accadeva su questo pulsante.

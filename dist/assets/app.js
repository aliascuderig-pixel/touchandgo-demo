...
function BookedScreen() {
  ...
  const qrData = encodeURIComponent(
    `TouchAndGo|${state.bookingCode}|ritiro:${state.pickupPoint}|dest:${state.destination}`
  );
...
function IdentifyScreen() {
  ...
  const destField = el("div", "dest-field");
  destField.innerHTML = `
    <div class="dest-lbl">Destinazione finale</div>
    <select class="dest-select" id="dest-select-identify">
      ${DESTINATIONS.map((d) => `<option value="${d.name}">${d.name}</option>`).join("")}
    </select>`;
  wrap.appendChild(destField);

  const goBtn = el("button", "btn-primary", "Salva e continua →");
  goBtn.addEventListener("click", () => {
    const name = document.getElementById("name-input").value.trim();
    const dest = document.getElementById("dest-select-identify").value;
...
function saveProfile() {
  try {
    localStorage.setItem(
      "tg_profile",
      JSON.stringify({ name: state.touristName, destination: state.destination })
    );
  } catch (e) {}
}
